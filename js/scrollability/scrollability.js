...
function wrapTarget(target, startX, startY, startTime) {
    ...
    function update(pos, continues) {
		...
		var offset = target.node.offsetTop;
		var scrollmin = max + offset;

        var range = -min - max;
        if (scrollbar && viewport < range) {
            var viewable = viewport - kScrollbarMargin*2;
            var height = (viewable/range) * viewable;
            var scrollPosition = scrollmin + kScrollbarMargin;
            if (position > max) {
...
function createYTarget(element) {
	...
	// remove y-offset betwee parent and element
	var viewport = parent.offsetHeight - element.offsetTop;
